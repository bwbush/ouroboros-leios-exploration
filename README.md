...
| How would an adversary turn a dollar budget into an attack, and when does one attack dominate another? | [The economics of attack resources](./assessments/attack-resource-economics.md) — a resource catalog, a budget-constrained price model, and a dominance relation, built on existing game-theoretic frameworks |
| Can current evidence establish which Leios attacks dominate others, and what would each cost an adversary? | [A dominance sketch for Leios attacks](./artifacts/leios-attack-dominance-sketch.md) — a prerequisite and evidence map over the threat model's T-numbers, with the missing comparisons made explicit |
